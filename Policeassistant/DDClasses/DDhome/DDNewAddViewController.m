...
@interface DDNewAddViewController ()<UITableViewDataSource,UITableViewDelegate>
{
    NSString *urlStr;
...
@implementation DDNewAddViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    DataArray=[[NSMutableArray alloc] init];
    urlStr=[NSString stringWithFormat:@"%@%@",API_BASE_URL(@"v1/site/transferrecordquery?type="),_type];
    ...
    self.tableView.rowHeight=147.5;
...
- (UITableViewCell * )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DDNewAddTableViewCell*cell=[tableView dequeueReusableCellWithIdentifier:ID];
    if (cell==nil) {
        cell=[[DDNewAddTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:ID];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    vc.typestr=[NSString stringWithFormat:@"通知"];
