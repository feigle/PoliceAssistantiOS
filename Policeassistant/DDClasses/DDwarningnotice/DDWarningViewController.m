...
#import "DDWarningViewController.h"
#import "DDWarningTableViewCell.h"
#import "DDPersonnelViewController.h"
#import "AppDelegate.h"

@interface DDWarningViewController ()<UITableViewDataSource,UITableViewDelegate>
{
    NSString *urlStr;
...
@implementation DDWarningViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"预警通知";
    DataArray=[[NSMutableArray alloc] init];
    urlStr=[NSString stringWithFormat:@"%@",API_BASE_URL(@"v1/site/notifyrecord")];
    DataArray=[DDNetCache cacheJsonWithURL:urlStr];
    [self getNetWorkData];
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, [UIScreen mainScreen].bounds.size.height-64-49) style:UITableViewStylePlain];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if (self.appDelegate.is_unread_message) {
        [_tableView.mj_header beginRefreshing];
        self.appDelegate.is_unread_message=NO;
    }
}
...
- (UITableViewCell * )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DDWarningTableViewCell*cell=[tableView dequeueReusableCellWithIdentifier:ID];
    if (cell==nil) {
        cell=[[DDWarningTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:ID];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    vc.roomid=[NSString stringWithFormat:@"%@",Dic[@"room_number_id"]];
    vc.typestr=[NSString stringWithFormat:@"首页"];
