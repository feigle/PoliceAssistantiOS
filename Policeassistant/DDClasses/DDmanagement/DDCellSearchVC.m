...
@interface DDCellSearchVC ()<UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate>
{
    NSString *urlStr;
...
@implementation DDCellSearchVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title=_nagtiontitle;
    DataArray=[[NSMutableArray alloc] init];
 //   urlStr=[NSString stringWithFormat:@"%@%@",API_BASE_URL(@"v1/site/unit?buildid="),_buildid];
    ...
    self.searchBar.placeholder=@"请输入单元数，例：1单元";
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    if (getStr.length == 0) {
        ...
        for (int i=0; i<DataArray.count; i++) {
            ...
            NSString *str1=dic[@"unitname"];
            NSString *str2=dic[@"unitid"];
            NSRange range=[str1 rangeOfString:getStr];
            if (range.location!=NSNotFound) {
                NSDictionary *dict=[NSDictionary dictionaryWithObjectsAndKeys:str1,@"unitname",str2,@"unitid", nil];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text=dic[@"unitname"];
    return cell;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    vc.buildid=[NSString stringWithFormat:@"%@",Dic[@"unitid"]];
    vc.nagtiontitle=[NSString stringWithFormat:@"%@",Dic[@"unitname"]];
...
- (void)getNetWorkData{
  ...
  //  NSMutableDictionary *dicts = [NSMutableDictionary dictionaryWithObjectsAndKeys:_buildid,@"buildid",nil];
