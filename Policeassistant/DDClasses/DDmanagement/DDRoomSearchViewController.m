...
@interface DDRoomSearchViewController ()<UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate>
{
    NSString *urlStr;
...
@implementation DDRoomSearchViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title=_nagtiontitle;
    NSLog(@"buildid==%@",_buildid);
    DataArray=[[NSMutableArray alloc] init];
  //  urlStr=[NSString stringWithFormat:@"%@%@",API_BASE_URL(@"v1/site/room?unitid="),_buildid];
    urlStr=[NSString stringWithFormat:@"%@%@",API_BASE_URL(@"v2/site/getunitandroombybuild?buildid="),_buildid];
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 50, self.view.frame.size.width, [UIScreen mainScreen].bounds.size.height-158) style:UITableViewStylePlain];
    ...
    self.searchBar.placeholder=@"请输入房间号，例：504";
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    if (getStr.length == 0) {
        ...
        for (int i=0; i<DataArray.count; i++) {
            ...
            NSString *str1=dic[@"room"];
            NSString *str2=dic[@"roomid"];
            NSString *str3=dic[@"count"];
            NSRange range=[str1 rangeOfString:getStr];
            if (range.location!=NSNotFound) {
                NSDictionary *dict=[NSDictionary dictionaryWithObjectsAndKeys:str1,@"room",str2,@"roomid",str3,@"count", nil];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text=[NSString stringWithFormat:@"%@室",dic[@"room"]];
    cell.detailTextLabel.text=[NSString stringWithFormat:@"%@人",dic[@"count"]];
    cell.detailTextLabel.font=sysFont15;
    return cell;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary *Dic=[_subDataArray objectAtIndex:indexPath.row];
    DDRoomDetailViewController*vc=[[DDRoomDetailViewController alloc]init];
    vc.roomid=[NSString stringWithFormat:@"%@",Dic[@"roomid"]];
    vc.nagtiontitle=[NSString stringWithFormat:@"%@室",Dic[@"room"]];
    [self.navigationController pushViewController:vc animated:YES];
}
- (void)getNetWorkData{
