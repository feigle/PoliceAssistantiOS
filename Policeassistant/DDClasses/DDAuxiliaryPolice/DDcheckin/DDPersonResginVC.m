...
//  Policeassistant
//
//  Created by DoorDu on 16/8/3.
//  Copyright © 2016年 DoorDu. All rights reserved.
//

#import "DDPersonResginVC.h"
#import "MHDatePicker.h"

@interface DDPersonResginVC ()
...
@property (nonatomic, strong) DDTextView * NewAddressTv;

@end

@implementation DDPersonResginVC
...
- (void)viewDidLoad {
    ...
    headView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, KScreenWidth, 760)];
...
- (void)creatTableheadview{
    ...
    [self setupOneTextfield:famliyTex WithFrame:CGRectMake(15, CGRectGetMaxY(cardTex.frame)+15, KScreenWidth-30, 44) placeholder:@"" keyboardType:UIKeyboardTypeDefault leftView:view3 typeString:@"名族"];
...
//    DDLabel *lab22=[[DDLabel alloc]initWithAlertViewHeight:16 mycolor:MAINTEX myfram:CGRectMake(100, 20, 20, 20) mytext:@"男"];
//    [mview addSubview:lab22];
//    
//    DDLabel *lab23=[[DDLabel alloc]initWithAlertViewHeight:16 mycolor:MAINTEX myfram:CGRectMake(185, 20, 20, 20) mytext:@"女"];
//    [mview addSubview:lab23];
    ...
    _NewAddressTv = [[DDTextView alloc]initWithFrame:CGRectMake(15, CGRectGetMaxY(xview.frame)+15, KScreenWidth-30, 67)];
    _NewAddressTv.backgroundColor = [UIColor whiteColor];
    _NewAddressTv.delegate = self;
    _NewAddressTv.font = [UIFont systemFontOfSize:16.f];
    _NewAddressTv.textColor = MAINTEX;
    _NewAddressTv.textAlignment = NSTextAlignmentLeft;
    _NewAddressTv.editable = YES;
    _NewAddressTv.scrollEnabled = YES;    //当文字超过视图的边框时是否允许滑动，默认为“YES”
    _NewAddressTv.layer.cornerRadius = 5.0f;
    _NewAddressTv.layer.borderColor = LINECOLOR.CGColor;
    _NewAddressTv.layer.borderWidth = 0.5;
    _NewAddressTv.placeholderColor = TEXCOLOR;
    _NewAddressTv.placeholder = @"现居住地";
    [headView addSubview:_NewAddressTv];
    
    
    UIView *birView1=[[UIView alloc]initWithFrame:CGRectMake(15, CGRectGetMaxY(_NewAddressTv.frame)+15, KScreenWidth-30, 44)];
    ...
    UILabel *peopleCountLab=[[UILabel alloc]initWithFrame:CGRectMake(0, 15, KScreenWidth, 20)];
  ...
  //  [xxview addSubview:peopleCountLab];
    ...
    for (int i = 0; i < content.length; i ++) {
        ...
        if ([number containsObject:a]) {
            [attributeString setAttributes:@{NSForegroundColorAttributeName:RGB(255, 150, 0),NSFontAttributeName:[UIFont systemFontOfSize:14],NSFontAttributeName:[NSNumber numberWithInteger:NSUnderlineStyleSingle]} range:NSMakeRange(i, 1)];
        }
    }
    peopleCountLab.attributedText=attributeString;
...
- (void)sendToSerce{
    if ([nameTex.text isEqualToString:@""]) {
        ...
        return;
    }else if ([famliyTex.text isEqualToString:@""]){
        ...
        return;
    }else if ([_OldAddressTv.text isEqualToString:@""]){
        [DDProgressHUD showCenterWithText:@"请输入户籍地址！" duration:1.0];
        return;
    }else if ([_NewAddressTv.text isEqualToString:@""]){
        [DDProgressHUD showCenterWithText:@"请输入现居住地！" duration:1.0];
        return;
...
- (void)getNetWorkData{
    ...
    NSMutableDictionary *dicts = [NSMutableDictionary dictionaryWithObjectsAndKeys:nameTex.text,@"name",sexStr,@"sex",famliyTex.text,@"nation",cardTex.text,@"idnum",birthLab.text,@"birthday",_OldAddressTv.text,@"address",nowLab.text,@"intime",_NewAddressTv.text,@"nowaddress",nil];
