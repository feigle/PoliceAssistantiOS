...
@interface DDChooseDanVC ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation DDChooseDanVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title=@"楼栋房号";
    DataArray=[[NSMutableArray alloc] init];
    urlStr=[NSString stringWithFormat:@"%@%@",API_BASE_URL(@"v1/site/unit?buildid="),_type_id];
...
- (void)creatTableheadview{
    ...
    DDLabel *lab22=[[DDLabel alloc]initWithAlertViewHeight:18 mycolor:MAINTEX myfram:CGRectMake(15, 0, 200, 48) mytext:_unittring];
...
- (UITableViewCell * )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text=[NSString stringWithFormat:@"%@",dic1[@"unitname"]];
    cell.textLabel.font=sysFont18;
    cell.textLabel.textColor=MAINTEX;
    return cell;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    DDChooseRoomVC *vc=[[DDChooseRoomVC alloc]init];
    NSDictionary *dic1=[DataArray objectAtIndex:indexPath.row];
    vc.roomstring=[NSString stringWithFormat:@"%@-%@",_unittring,dic1[@"unitname"]];
    vc.type_id=[NSString stringWithFormat:@"%@",dic1[@"unitid"]];
