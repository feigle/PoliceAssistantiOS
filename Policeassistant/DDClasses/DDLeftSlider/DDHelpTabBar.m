...
@interface DDHelpTabBar ()

@end


@implementation DDHelpTabBarButton
...
-(void)initwithData
{
    ...
    //控制器数组(不需要导入控制器头文件)
    self.controllerArray = @[@"DDMissionDealVC",@"DDAlarmListVC",@"DDChooseComVC"];
    //title数组
    self.titleArray = @[@"任务处理",@"报警申报",@"人员登记"];
    //默认图片数组
    self.imageArray = @[@"order_normal",@"police_normal",@"people_normal"];
    //选中图片数组
    self.selImageArray = @[@"order_height",@"police_height",@"people_height"];
    //tabBar高度
...
-(void)createTabBarView
{
    ...
    for(int i=0;i<num;i++)
    {
        DDHelpTabBarButton *button = [[DDHelpTabBarButton alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/num*i, 0, [UIScreen mainScreen].bounds.size.width/num,self.tabBarHeight)];
        button.tag = 1000+i;
        ...
        button.backgroundColor = RGB(33, 34, 50);
