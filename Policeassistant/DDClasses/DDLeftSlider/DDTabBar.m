...
//title选中颜色
#define TitleColor_Sel  RGBCOLOR(51, 90, 220)
//title字体大小
...
-(void)initData
{
    ...
    //控制器数组(不需要导入控制器头文件)
    self.controllerArray = @[@"DDHomeViewController",@"DDWarningViewController",@"DDTaskassignmentViewController",@"DDManagementViewController"];
    //title数组
    self.titleArray = @[@"首页",@"预警通知",@"任务委派",@"社区管理"];
    //默认图片数组
    self.imageArray = @[@"tabbar_home_normal",@"tabbar_police_normal",@"tabbar_assmangt_normal",@"tabbar_manger_normal"];
    //选中图片数组
    self.selImageArray = @[@"tabbar_home_height",@"tabbar_police_height",@"tabbar_assmangt_height",@"tabbar_manger_height"];
    //tabBar高度
...
-(void)createTabBarView
{
    ...
    for(int i=0;i<num;i++)
    {
        DDTabBarButton *button = [[DDTabBarButton alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/num*i, 0, [UIScreen mainScreen].bounds.size.width/num,self.tabBarHeight)];
        button.tag = 1000+i;
