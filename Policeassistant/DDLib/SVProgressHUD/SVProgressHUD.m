...
+ (void)showWithStatus:(NSString *)status {
    [[SVProgressHUD sharedView] showWithStatus:status maskType:SVProgressHUDMaskTypeBlack networkIndicator:NO];
}
...
//自定义
+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType hasBgColor:(BOOL)hasBgColor{
    [[SVProgressHUD sharedView] showWithStatus:status maskType:maskType networkIndicator:NO hasBgColor:hasBgColor];
}
#pragma mark ----------自定义--------------
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show hasBgColor:(BOOL)hasBgColor{
...
//            NSLog(@"------------->");
//        }
//        NSLog(@"------------->%f,%f",KScreenBoundsHeight,KScreenWidth);
        //
...
//        if (KScreenBoundsHeight<KScreenWidth) {
//            self.hudView.center = CGPointMake(KScreenBoundsHeight*0.5, KScreenWidth*0.5f);
//        }
        
        if(self.alpha != 1) {
            [self registerNotifications];
            NSLog(@"rect = %@",NSStringFromCGRect(self.hudView.frame));
...
- (void)setStatus:(NSString *)string {
    ...
    //self.stringLabel.backgroundColor = [UIColor blackColor];
