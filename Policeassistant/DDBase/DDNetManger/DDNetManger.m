...
@interface DDNetManger ()
@end

@implementation DDNetManger
#pragma mark  － app代理
...
{
    static DDNetManger *manager = nil;
                                ...
                                progress:(DDDownloadProgress)progress
{
    ...
    NSLog(@"请求头: %@\n请求方式: %@\n请求URL: %@\n请求param: %@\n\n",[self sharedAFManager].requestSerializer.HTTPRequestHeaders, (type == DDHttpRequestTypeGet) ? @"POST":@"GET",URLString, parameters);
    
    DDURLSessionTask *sessionTask = nil;
    
    if (type == DDHttpRequestTypeGet)
    {
        sessionTask = [[self sharedAFManager] GET:URLString parameters:parameters  progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            NSNumber *need_devicetoken =[dic objectForKey:@"need_devicetoken"];
            NSString *devce=[NSString stringWithFormat:@"%@",need_devicetoken];
            if ([Error_Code_Failed isEqualToString:devce]) {
                [DDTools sendDeviceToken:[DDUserDefault getDeviceToken]];
            }
            successBlock(responseObject);
            
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
            NSDictionary *dic1=error.userInfo;
            NSString *string1=[NSString stringWithFormat:@"%@",dic1[@"NSLocalizedDescription"]];
            NSString *string2=[NSString stringWithFormat:@"%@",dic1[@"NSErrorFailingURLKey"]];
            if([string1 rangeOfString:@"401"].location !=NSNotFound)
            {
                [SVProgressHUD dismiss];
                [DDProgressHUD showCenterWithText:@"账号密码错误,请重新登陆！" duration:1.0];
            ...
            }else if ([string2 rangeOfString:@"unreadmessage"].location !=NSNotFound){
                [SVProgressHUD dismiss];
              //  [DDProgressHUD showCenterWithText:@"不做任何提示！" duration:1.0];
            }
            else
            {
                [SVProgressHUD dismiss];
                [DDProgressHUD showCenterWithText:@"网络请求超时！" duration:1.0];
            }
            failureBlock(error);
    ...
    else if (type == DDHttpRequestTypePost)
    {
    ...
    else if (type == DDHttpRequestTypePut)
    {
    ...
    else if (type == DDHttpRequestTypeDelete)
    {
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status)
        {
            case AFNetworkReachabilityStatusUnknown: // 未知网络
                NSLog(@"未知网络");
                DDNetManagerShare.netWorkStatus = DDNetworkStatusUnknown;
                break;
            case AFNetworkReachabilityStatusNotReachable: // 没有网络(断网)
                NSLog(@"没有网络");
                DDNetManagerShare.netWorkStatus = DDNetworkStatusNotReachable;
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN: // 手机自带网络
                NSLog(@"手机自带网络");
                DDNetManagerShare.netWorkStatus = DDNetworkStatusReachableViaWWAN;
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi: // WIFI
                
                DDNetManagerShare.netWorkStatus = DDNetworkStatusReachableViaWiFi;
                NSLog(@"WIFI--%lu", (unsigned long)DDNetManagerShare.netWorkStatus);
                break;
        }
    }];
